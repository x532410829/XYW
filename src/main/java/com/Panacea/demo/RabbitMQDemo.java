...
/**
 * RabbitMQ 消息订阅发布demo， 首先pom文件导包，然后写配置文件，配置内容查看application.properties文件；
 * 这里配置MQ的交换机有3种，每一种都单独配置一个配置类来和对应的队列绑定，配置和绑定成功后，可以在 MQ的控制面板里面看到对应的交换机的队列的情况
 * 
 * @author 夜未
 * @since 2020年9月11日
 */
...
//=====================================直连型交换机===============================================================	
	/**
	 * Direct Exchange 配置类 直连型交换机，根据消息携带的路由键将消息投递给对应队列。
	 * 大致流程，有一个队列绑定到一个直连交换机上，同时赋予一个路由键 routing key 。
	 * 然后当一个消息携带着路由值为X，这个消息通过生产者发送给交换机时，交换机就会根据这个路由值X去寻找 绑定值也是X的队列
	 */
...
//=====================================扇型交换机===============================================================	
	...
	/**
	 * FanoutExchange 扇型交换机，这个交换机没有路由键概念，就算你绑了路由键也是无视的。 这个交换机在接收到消
...
//=====================================主题交换机===============================================================	
	/**
	 * Topic Exchange配置类 主题交换机，这个交换机其实跟直连交换机流程差不多，但是它的特点就是在它的路由键和绑定键 之间是 有规则的。
	 * 简单地介绍下规则： (星号) 用来表示一个单词 (必须出现的) # (井号) 用来表示任意数量（零个或多个）单词,
	 * 通配的绑定键是跟队列进行绑定的，举个小例子 队列Q1 绑定键为 *.TT.* 队列Q2绑定键为 TT.# 如果一条消息携带的路由键为
	 * A.TT.B，那么队列Q1将会收到； 如果一条消息携带的路由键为TT.AA.BB，那么队列Q2将会收到；
	 */
	 ...
	 * 分别订阅了topic.man和topic.woman 2个队列；交换机绑定的队列中有2种绑定键；一个为 topic.man另一个为topic.#
	 * ；由于这里的绑定键为topic.man，所以2个消费者都符合条件， 都可以接受到消息
	 * 
	 * @return
	 */
